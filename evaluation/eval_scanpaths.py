...
def scanpath_to_string(scanpath, height, width, Xbins=10, Ybins=10):
    ...
    for i in range(scanpath.shape[0]):
        fixation = scanpath[i]
        xbin = int(fixation[0] // width_step)
        ybin = int((height - fixation[1]) // height_step)
        corrs_x = chr(65 + xbin)
        corrs_y = chr(97 + ybin)
        if scanshape == 2:
            ...
            dbin = int(round(fixation[2] / 0.05))
            string += (corrs_y + corrs_x) * dbin
            num += [(ybin * Xbins) + xbin] * dbin
    return string, num
