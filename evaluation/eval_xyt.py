...
if args.scanpaths:
            ...
            for row in reader:
                ...
                if uniq != cur:
                    ...
                    obj = {'image': row['image'], 'width': int(row['width']), 'height': int(row['height']),
                           "username": row['username'], 'scanpath': [], 'duration': []}
                ...
                if x < 0 or x > 1:
                    x = min(max(x, 0.0001), 0.9999)
                if y < 0 or y > 1:
                    y = min(max(y, 0.0001), 0.9999)

                obj['scanpath'].append([x, y])

                t = row["timestamp"]
                obj["duration"].append(float(t))
    ...
    a_dtw, a_dtwd, a_tde, a_eye, a_fre, a_euc, a_hau, a_rec, a_det, a_lam, a_cor, a_lev, a_man, a_sma, a_smad = [], [], [], [], [], [], [], [], [], [], [], [], [], [], []
    ...
    for reference in ref_files:
        ...
        for r in ref_objs:
            key = "%s___%s" % (r['image'], r["username"])
            ref_images[key].append(r)
        ...
        for prediction in pred_files:
            ...
            for pred in pred_objs:
                ...
                refs = ref_images[pred['image'] + "___" + pred['username']]
                ...
                for ref in refs:
                    ...
                    rt_seq = np.array(ref['duration'])[:, np.newaxis]
                    pt_seq = np.array(pred['duration'])[:, np.newaxis]

                    rxyt_seq = np.concatenate((r_seq, rt_seq), -1)
                    pxyt_seq = np.concatenate((p_seq, pt_seq), -1)
                    ...
                    dtwd = scan.DTW(pxyt_seq, rxyt_seq)
                    ...
                    lev = scan.levenshtein_distance(p_seq, r_seq, height, width)

                    # FIXME: ScanMatch implementation is buggy.

                    p_seq_raw = p_seq * np.array([[width, height]])
                    r_seq_raw = r_seq * np.array([[width, height]])

                    pxyt_seq_raw = pxyt_seq * np.array([[width, height, 1]])
                    rxyt_seq_raw = rxyt_seq * np.array([[width, height, 1]])

                    sma = scan.scan_match(p_seq_raw, r_seq_raw, height, width)
                    smad = scan.scan_match(pxyt_seq_raw, rxyt_seq_raw, height, width)
