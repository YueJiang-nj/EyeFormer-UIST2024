...
def truncate_coord(coord, max_words):
    if len(coord) >= max_words:
        return coord[:max_words]

    else:
        pad_coord = [[0, 0]] * (max_words - len(coord))
        new_coord = coord + pad_coord

        return new_coord


def truncate_time(time, max_words):
    if len(time) >= max_words:
        return time[:max_words]

    else:
        pad_coord = [0.] * (max_words - len(time))
        new_coord = time + pad_coord
